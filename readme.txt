=== Plugin Name ===
Contributors: excuseser
Tags: html5,audio,video,mp4,mp3
Requires at least: 2.5.0
Tested up to: 2.9.1
Stable tag: 1.5.1

A plugin for WordPress to display html5 videos. Shortcodes for HTML5 video and audio.

== Description ==

A plugin for WordPress to display html5 videos.Shortcodes for HTML5 video and audio.
can't be work with Degradable HTML5 audio and video. so sorry, i will resolved it as soon as possible

= Feedback =


== Installation ==

1. Search for `html5 video` in the `Plugins` menu to install.
1. Or upload folder html5video to the `/wp-content/plugins/` directory.
1. Activate html5 video through the 'Plugins' menu in WordPress.
1. Place the code such as `[video src="http://www.sample.com/xxx.mp4"]` in your post, or just use the `media Insert` in TinyMCE, and then publish it.

= Usage =
1. In TinyMCE:
Click the icon of html5video, in the popup window, there're 2 tabs.